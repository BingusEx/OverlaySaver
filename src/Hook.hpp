		...
		void write_thunk() {
			...
			if (opcode == 0xE8) {  // CALL instruction
				...
				if ((opcode & rexw) != rexw) {  // REX.W Must be present for a valid 64-bit address replacement.
					SKSE::stl::report_and_fail("Invalid hook location, lea instruction must use 64-bit register (first byte should be between 0x48 and 04F)"sv);
				}
				...
				if (opcode == 0x8D) {                  // LEA instruction
					...
					SKSE::stl::report_and_fail("Invalid hook location, write_thunk can only be used for call or lea instructions"sv);
				}
			}
		}
